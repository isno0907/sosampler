...
class BaseRecognizer(nn.Module, metaclass=ABCMeta):
    ...
    @auto_fp16()
    def extract_feat(self, imgs, is_resnet=False):
        ...
        """
        if is_resnet:
            x = self.backbone(imgs, is_training=True)
        else:
    ...
    def average_clip(self, cls_score, num_segs=1):
        ...
        which defined in test_cfg) to compute the final averaged
    ...
    def train_step(self, data_batch, optimizer, **kwargs):
        ...
        output = self(imgs, label, return_loss=True, **aux_info)
        losses = output
