...
class X3D(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.is_sampler:
            temporal = ret.shape[2]
            num_batches = ret.shape[0]
            ret = ret.transpose(1, 2).contiguous()
            ret = ret.reshape((-1,) + ret.shape[-3:])
            ret = self.avg_pool(ret)
            ret = ret.squeeze()

            ret = ret.reshape((num_batches, temporal, self.out_channel))
            logit = self.logit(ret).squeeze(-1)
            assert torch.isnan(logit).sum() == 0
            probs = F.softmax(logit, dim=1)
            assert torch.isnan(probs).sum() == 0
            return probs
        return ret
