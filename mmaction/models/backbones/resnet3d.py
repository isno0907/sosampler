...
class ResNet3d(nn.Module):
    ...
    def forward(self, x, is_training=False):
        ...
        """
        # B 3 sampled_seg 224 224 -> B*seg 3 1 h w
        if is_training:
            x = rearrange(x, 'b c t h w -> (b t) c 1 h w') #training때 추가
        # print(x.shape)
        ###
        x = self.conv1(x)
        # print(f'After conv:{x.shape}')
        # 128 64 sampled_seg 112 112
        x = self.maxpool(x)
        outs = []
        ...
        if len(outs) == 1: # Yes 
            return outs[0] # -> B 2048 5 7 7 or BT 2048 1 7 7
