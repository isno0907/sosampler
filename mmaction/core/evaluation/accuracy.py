...
def top_k_accuracy(scores, labels, topk=(1, ), multi_class=False):
    ...
    #print(f"scores = {np.array(scores).shape}")
    ...
    if multi_class:
        ...
        max_preds = np.argsort(scores, axis=1)
        #logger.info(f"max_preds = {max_preds[0]}")
...
def top_k_class(scores, labels, k=30):
    pred = np.argmax(scores, axis=1)
    cf_mat = confusion_matrix(pred, labels).astype(float)
    ...
    hit_ratio = np.array([hit / cnt if cnt else 0.0 for cnt, hit in zip(cls_cnt, cls_hit)])
