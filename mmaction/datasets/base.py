...
from ..core import (mean_average_precision, mean_class_accuracy,
                    mmit_mean_average_precision, top_k_accuracy, top_k_class)
...
class BaseDataset(Dataset, metaclass=ABCMeta):
    ...
    def evaluate(self,
        ...
        allowed_metrics = [
            ...
            'mmit_mean_average_precision', 'top_k_class'
        ]
        ...
        for metric in metrics:
            ...
            if metric == 'top_k_class':
                results = top_k_class(results, gt_labels, 30)
                log_msg=[]
                for pair in results:
                    index, value = pair
                    eval_results[f'label_{index}_acc_ratio'] = value
                    log_msg.append(f'\nlabel_{index}_acc_ratio\t{value:.4f}')
            ...
            if metric in [
                ...
                if metric == 'mean_average_precision':
                    mAP = mean_average_precision(results, gt_labels_)
                elif metric == 'mmit_mean_average_precision':
                    mAP = mmit_mean_average_precision(results, gt_labels_)
    ...
    def __getitem__(self, idx):
        ...
        if self.test_mode:
            for num_retry in range(self.retries):
                ...
                print(f'Failed to load samples after {self.retries} retries')
